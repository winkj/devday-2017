...
LWiFiClient c; SensirionESS ess;
...
void setup() { 
  ...
  Serial.println("Connecting to AP " WIFI_AP); 
...
void loop() { 
  ...
  sendAndRead(temp, rh, tvoc, t_alert); 
...
void sendAndRead(float temp, float rh, float voc, float t_alert) { 
  ...
  c.print("?temp="); c.print(temp); 
  c.print("&rh="); c.print(rh); 
  c.print("&tvoc="); 
  c.print(voc); 
  c.print("&t_alert="); 
  c.print(t_alert); 
  ...
  c.println(); // waiting for server response 
